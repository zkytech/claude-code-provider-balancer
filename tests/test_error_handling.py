...
def test_openai_error_mapping(client, status_code, openai_error, anthropic_error_type):
    ...
    mock_request = httpx.Request("POST", "https://test.openrouter.ai/api/v1/chat/completions")
    ...
    with patch("claude_proxy.api.client.chat.completions.create", side_effect=api_error):
        request_data = {
        ...
        assert openai_error in data["error"]["message"], f"Expected error message to contain '{openai_error}'"
...
def test_network_error_mapping(client, exception, expected_status, anthropic_error_type):
    """Test mapping of network-level errors to Anthropic error format."""
    
    mock_request = httpx.Request("POST", "https://test.openrouter.ai/api/v1/chat/completions")
    
    if isinstance(exception, (httpx.ConnectError, httpx.ReadError, httpx.ConnectTimeout)):
        api_error = openai.APIConnectionError(message=str(exception), request=mock_request)
    else:
        api_error = exception
...
def test_streaming_rate_limit_error(client):
    ...
    with patch("claude_proxy.api.client.chat.completions.create", side_effect=streaming_error):
        request_data = {
            "model": "claude-3-opus-20240229",
            "messages": [{"role": "user", "content": "Hello"}],
            "max_tokens": 1024,
...
def test_streaming_connection_error(client):
    ...
    with patch("claude_proxy.api.client.chat.completions.create", side_effect=connection_error):
        request_data = {
            "model": "claude-3-opus-20240229",
            "messages": [{"role": "user", "content": "Hello"}],
            "max_tokens": 1024,
